...
extension keychainClass where Self : NSObject {
    ...
    func updataValue(accessKey: String, classKey: String, valueData: String) {
        ...
        let resultCode = SecItemUpdate(queryUpdate as CFDictionaryRef, queryUpdate as CFDictionaryRef)
        
        if resultCode != noErr {
            print("키체인 업데이트 하다 에러발생 : 에러코드 \(resultCode)")
        }
...
// 에러코드 -25243의 경우는 키체인 활성화를 안시킨것임 (키체인 엑세스 거부!)
